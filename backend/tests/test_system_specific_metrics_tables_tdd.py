...
- market_data_usage_metrics: For tracking market data provider API calls
...
class TestSystemSpecificMetricsTables:
    ...
    def test_market_data_usage_metrics_table_should_exist(self, db: Session):
        """
        Test that market_data_usage_metrics table exists for market data provider tracking.

        Given: Market data system with multiple providers (Alpha Vantage, yfinance, etc.)
        When: Inspecting database schema
        Then: market_data_usage_metrics table should exist
        ...
        # Check that market_data_usage_metrics table exists
        assert "market_data_usage_metrics" in table_names, \
            "market_data_usage_metrics table is missing - required for market data provider tracking"

        # Verify table has required columns for market data provider metrics
        columns = inspector.get_columns("market_data_usage_metrics")
        ...
        for required_col in required_columns:
            assert required_col in column_names, \
                f"Column '{required_col}' missing from market_data_usage_metrics table"
    ...
    def test_market_data_models_use_correct_table_name(self):
        ...
        Then: Models should reference 'market_data_usage_metrics' table
        ...
        expected_table_name = "market_data_usage_metrics"
        actual_table_name = MarketDataUsageMetrics.__tablename__

        assert actual_table_name == expected_table_name, \
            f"MarketDataUsageMetrics model uses table '{actual_table_name}' but should use '{expected_table_name}'"